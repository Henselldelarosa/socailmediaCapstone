...
@login_required
def get_all_files():
    files = Image.query.order_by(Image.id.desc()).all()
    return {'files': [file.to_dict() for file in files]}

@image_routes.route("/post_images")
@login_required
def get_all_images():
